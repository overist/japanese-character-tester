...
import { katakanas } from "@/datas/katakanas";

export default function Katakana() {
  // ** states and conditions
  const inputRef = React.useRef<HTMLInputElement>(null);
  const [katakanaTest, setKatakanaTest] = useRecoilState(katakanaTestState);
  const { stage, mode, quizs } = katakanaTest;
  ...
  const handleSubmit = () => {
    ...
    if (inputData === answer) {
      ...
      setKatakanaTest({
        ...katakanaTest,
        wrongs: [...katakanaTest.wrongs, quizs[stage - 1]],
      });
    }
  };
    ...
    return (
      <TestStatusWrap>
        ...
        <button
          className="reset"
          onClick={() =>
            setKatakanaTest({ stage: 0, mode: 0, wrongs: [], quizs: [] })
          }
        >
          Reset
        </button>
